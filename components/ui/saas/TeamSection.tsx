  ...
  return (
    <section className={`py-16 md:py-24 ${className}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`grid ${gridClass[columns]} gap-8`}>
          {members.map((member, index) => (
            <Card key={index} className="group overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 bg-white dark:bg-gray-900">
...
function SocialLinks({ links }: { links: TeamMember['socialLinks'] }) {
  if (!links) return null

  const socialItems = [
    { key: 'twitter', icon: TwitterIcon, url: links.twitter },
    { key: 'linkedin', icon: LinkedInIcon, url: links.linkedin },
    { key: 'github', icon: GitHubIcon, url: links.github },
    { key: 'email', icon: EmailIcon, url: links.email ? `mailto:${links.email}` : undefined },
